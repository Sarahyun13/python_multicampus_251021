...
print(strA.upper()) #영문 문자열 대문자 변환
print(strA.capitalize()) #영문 문자열 첫 글자 대문자 변환
print(strB.replace("강력해", "매우 강력해")) #한글 문자열 치환

data = "<<<< spam and ham >>>>"
result = data.strip("<> ") #문자열 양쪽 공백과 특정 문자 제거
print(data)
print(result) #제거된 결과 출력

#문자열을 리스트로 변환
lst = result.split()
print(lst) #리스트 출력
#다시 하나로 합치기
print(":)".join(lst)) #리스트를 문자열로 합쳐서 출력
